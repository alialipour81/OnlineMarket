...
@section('content')
    ...
    <div class="card-body">
        <form action="{{ isset($product) ? route('user-products.update',$product->title_fa) : route('user-products.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            @isset($product)
                @method('PUT')
                @php
            ...
            <div class="form-group mb-3">
                <label for="1" class="mb-1"> فروشگاه خود را انتخاب کنید:</label>
                <select name="market_id" id="1" class="form-control @error('market_id') is-invalid @enderror">
                        @foreach($countMarkets as $market)
                            <option value="{{ $market->id }}"  @isset($product) @if($forosh == $market->name) selected @endif @endisset>{{ $market->name }}</option>
                        @endforeach
                </select>
                @error('market_id')
            ...
            <div class="form-group mb-3">
                <input type="hidden" id="1" class="form-control " placeholder="نام فروشنده محصول   " name="forosh"  value="{{ isset($product) ? $forosh : '0' }}" >
            </div>
            <div class="form-group mb-3">
